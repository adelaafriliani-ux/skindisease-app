...
# ============================================================
def predict_with_tta(model, base_img_array):
    variations = [
        base_img_array,
    ...
    batch = np.stack(variations, axis=0)
    all_preds = model.predict(batch, verbose=0)
    return np.mean(all_preds, axis=0)

@st.cache_resource
def load_classifier():
    model = load_model("model_skin_disease.keras")
...
try:
    ...
    if current_l1 == "🔬 Deteksi":
        ...
        if not MODEL_LOADED:
            st.error(f"Model belum bisa dimuat. Pastikan file 'model_skin_disease.keras' dan 'class_names.json' berada di folder yang sama dengan app.py.\n\nDetail error: {LOAD_ERROR}")
            st.stop()
